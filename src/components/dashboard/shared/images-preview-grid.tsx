				...
				const palettes = await Promise.all(
					images.map(async (img, index) => {
						try {
							...
							console.warn(`Failed to extract colors from image ${index}:`, error);
							return [];
						}
					})
				);
	...
	if (imagesLength === 0) {
		return (
				<div className="max-w-4xl">
					<div className="w-full h-[400px] bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl border-2 border-dashed border-gray-300 flex flex-col items-center justify-center transition-colors duration-200 hover:border-gray-400 hover:bg-gradient-to-br hover:from-gray-100 hover:to-gray-150">
						<div className="p-4 bg-white rounded-full shadow-lg mb-4">
							<ImageIcon className="h-12 w-12 text-gray-400" />
						</div>
						<h3 className="text-xl font-semibold text-gray-600 mb-2">Nessuna immagine caricata</h3>
						<p className="text-sm text-gray-500 text-center max-w-sm">
							Carica almeno 3 immagini di alta qualità per mostrare il tuo prodotto nel modo migliore
						</p>
				...
				<div className="max-w-4xl">
					...
					<div className="flex items-center justify-between mb-4">
						<div className="flex items-center gap-3">
							...
							<div>
								<h3 className="font-semibold text-gray-900">Anteprima Immagini</h3>
								<p className="text-sm text-gray-600">Passa sopra un&apos;immagine per vedere le opzioni</p>
							</div>
						</div>
							...
							<div
								...
								{imageErrors.has(i) ? (
									<div className="w-full h-full flex flex-col items-center justify-center text-red-500">
										...
										<span className="text-xs font-medium">Errore caricamento</span>
									</div>
								) : (
									<Image
										src={img.url}
								...
								<div className={cn(
									...
									{colorPalettes[i] && colorPalettes[i].length > 0 && (
										<div className="bg-white/95 backdrop-blur-sm rounded-lg p-3 shadow-lg">
											<div className="flex items-center gap-2 mb-2">
												...
												<span className="text-xs font-medium text-gray-700">Colori estratti</span>
											</div>
											<ColorPalette
												colors={colors}
												setColors={setColors}
									...
									<div className="flex items-center gap-2">
										<Button
											size="sm"
											variant="secondary"
											className="bg-white/90 hover:bg-white text-gray-700 shadow-md flex items-center gap-1"
											onClick={(e) => {
												e.preventDefault();
